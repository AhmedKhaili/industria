# P8 — Intégration export complet traçabilité (client démo)

> **Statut** : Phase I0–I1 livrée (`e053e11`) — plan tests `df_propre` intégré avant I2  
> **Branche** : `feat/demo-traceability-export-integration`  
...
| Usage actuel | `dataset.fichier` par défaut ; tests S2 client démo ; campagne F2 compact v1 validée |
...
| Colonnes | **18** |
| Format | LONG (`Tag`, `Value`) |
| Taille / volume | Export volumineux local non versionné (périmètre plus riche que l’ancien export) |
...
**Fichier local (gitignored)** : `data/lisi_capteurs_export_complet_tracabilite.csv`  
...
`configs/lisi_aerospace/client_config_traceability.yaml` — copie UTF-8 de la config démo aéronautique avec `dataset.fichier` pointant vers l’export traçabilité.
...
1. [ ] Valider que `data/lisi_capteurs_export_complet_tracabilite.csv` est présent localement (export volumineux non versionné).
